...
class Board:
    ...
    def draw_board_spaces(self):
        for i in list(range(0, 10)) + list(range(10, 20)) + list(range(20, 30)) + list(range(30, 36)):
            ...
            self.squares.append(Square(i-1))
    ...
    def update_player_position(self, player):
        raio = LARGURA_CASA // 6
        x, y = get_board_house_coordinates(player.get_position())
        i = self.game.get_players().index(player)
        ...
        centro_x, centro_y = x + raio, y + raio
        self.canvas.coords(player.get_char(), centro_x - raio, centro_y - raio, centro_x + raio, centro_y + raio)
