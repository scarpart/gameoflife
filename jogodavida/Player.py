...
class Player(object):
	def __init__(self, player_name, player_id, priority, colour, player_interface):
		...
		self.__can_roll_dice = False # FIXME: should be false for new players (testing purposes here)
		self.__can_select_card = False
		self.__can_pick_career = False
		self.__can_select_property = False

		self.set_player_state(PlayerState.READY_TO_PLAY) # Ver diagramas de estados

	def set_player_state(self, state : PlayerState):
		if state == PlayerState.READY_TO_PLAY:
			self.__is_first_move = True
			self.__can_roll_dice = True
			self.__can_select_property = False
			self.__can_pick_career= False
			self.__can_select_card = False			
		elif state == PlayerState.SELECTING_CARD:
			self.__can_roll_dice = False
			self.__is_first_move = False
			self.__can_select_card = True
		elif state == PlayerState.SELECTING_PROPERTY:
			self.__can_select_card = False
			self.__can_select_property = True
		elif state == PlayerState.CHOSE_CARD:
			self.__can_select_property = False
		elif state == PlayerState.PICKING_CAREER:
			self.__is_first_move = False
			self.__can_pick_career = True
			self.__can_roll_dice = False
		elif state == PlayerState.NOT_TURN:
			self.__can_pick_career = False

	def get_can_select_property(self) -> bool:
		return self.__can_select_property

	def get_can_pick_career(self) -> bool:
		return self.__can_pick_career

	def get_paid_rent_amount(self) -> int:
		return self.__paid_rent_amount
	
	def set_paid_rent_amount(self, amount : int):
		self.__paid_rent_amount = amount
	...
	def to_dict_move(self) -> dict:
		move_dict = { 
			...
			"is_first_move": self.___is_first_move,
			"nfam_members": self.__nfam_members,
			"can_roll_dice": self.__can_roll_dice,
			"can_select_card": self.__can_select_card,
			"can_select_property": self.__can_select_property,
			"can_pick_career": self.__can_pick_career,
		}
		return move_dict

	def update(self, move : dict, rent_owner_name : str):
		...
		for key, val in move.items():
			if key == "career_attrs":
				...
				if val["paid_rent"]:
					message += f"He paid rent to {rent_owner_name} and lost US$ {val['paid_rent_amount']}. "
	...
	def update_normal_attrs(self, move : dict):
		...
		# TODO: this one
		self.__is_first_move = move["is_first_move"]
		self.__nfam_members = move["nfam_members"]
		self.__can_roll_dice = move["can_roll_dice"]
		self.__can_select_card = move["can_select_card"]
		self.__can_pick_career = move["can_pick_career"]
		self.__can_select_property = move["can_select_property"]
	...
	def modify_state(self, state):		
		if state.type == ModifierType.SALARY:
			...
			self.add_family_member(state.is_wife) 

	def add_family_member(self, is_wife : bool):
		if is_wife and self.__nfam_members < 1:
			self.__player_interface.show_messagebox_info("Marriage", f"Congratulations, you've been married! \
			...
			self.__player_interface.show_messagebox_info("Already married", "Fortunately (or unfortunately) for you, you are already married, and cannot have another wife. Skipping this turn.")
			return
		elif not is_wife and self.__nfam_members < 1:
			self.__player_interface.show_messagebox_info("Cannot have baby", "Unfortunately, you cannot have a baby if you have no wife. Skipping this turn.")
			return
		elif not is_wife and self.__nfam_members < 4:
			self.__player_interface.show_messagebox_info("Child birth", f"Congratulations, a child has been born in your family! \
			...
			self.__player_interface.show_messagebox_info("Family full!", "You already have a full family, therefore you cannot add new members. Skipping this turn.")
			return
	...
	def set_can_roll_dice(self, val: bool):
		self.__can_roll_dice = val

	def get_can_roll_dice(self) -> bool:
		return self.__can_roll_dice

	def get__is_first_move(self) -> bool:
	...
	def get_rent_owner(self) -> str:
		return self.__rent_owner
	
	def set_rent_owner(self, owner_id : str):
		self.__rent_owner = owner_id
	...
	def get__is_first_move(self) -> int:
		return self.__is_first_move
