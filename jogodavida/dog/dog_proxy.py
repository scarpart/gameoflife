...
class DogProxy:
    ...
    def initialize(self, a_name, an_actor):
        ...
        try:
            ...
            self.__game_id = config_file.read()
        except FileNotFoundError:
            self.status = 0
            return "Arquivo de configuração do jogo não encontrado"
        config_file.close()
        resp = self.register_player(self.player_name, self.player_id, self.__game_id)
    ...
    def start_match(self, number_of_players):
        url = self.url + "start/"
        post_data = {"player_id": self.player_id, "game_id": self.__game_id, "number_of_players": number_of_players}
        resp = requests.post(url, data=post_data)
        result = resp.status_code
        if result == 200:
            resp_json = resp.text
    ...
    def start_status(self):
        ...
        post_data = {"player_id": self.player_id, "game_id": self.__game_id}
        resp = requests.post(url, data=post_data)
        result = resp.status_code
    ...
    def send_move(self, a_move):
        ...
        post_data = {"player_id": self.player_id, "game_id": self.__game_id, "move": json_move}
        resp = requests.post(url, data=post_data)
